...
describe('register', () => {
  ...
  describe('git hook', () => {
    ...
    it('no git does nothing', () => testWithLogging(async log => {
      await outputFiles('.', {
    ...
    it('do not add git hook if another already exists', () => testWithLogging(async log => {
      await spawn('git', ['init'])
      await outputFiles('.', {
        'package.json': JSON.stringify({
          name: 'test',
    ...
    it('git hook run fails', () => testWithLogging({
      callback: async log => {
        ...
        await outputFiles({
          ...
          'node_modules/base-lang-standard/index.js': endent`
            module.exports = {
              eslintConfig: {
                "rules": {
                  "semi": ["error", "never"],
